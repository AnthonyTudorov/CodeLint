...
# pylint: disable=subprocess-run-check,missing-function-docstring,no-member, invalid-name, wrong-import-order
import os
import base64
import requests
...
def log_user_info(access_token):
    ...
    model = models.Users(login, name, email, profile_image, user_id, a)
...
def get_user_repos(user_id):
    user_access_token = Fernet(access_token_key).decrypt(
    ...
    repos = repos.json()
    return {
        'repos': [(repo['name'], repo['url'], repo['default_branch'])
                  for repo in repos],
        'error':
        None
    }
...
def commit_changes(user_id, repo_url, default_branch, files, commit_message):
    new_tree_sha, old_commit_sha = create_new_tree(user_id, repo_url,
